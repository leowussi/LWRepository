...
@interface LMapViewController ()<BMKPoiSearchDelegate,UISearchBarDelegate,mapBtndelegate,UIScrollViewDelegate,BMKGeneralDelegate>
{
...
- (void)viewDidLoad {
    ...
    strMapType = [user objectForKey:@"typeStr"];
    
    DLog(@"strMapType == %@",strMapType);
    
    if (self.tag == 10 || self.VCtag == 10000) {
        [self addNavigationLeftButton];
        //        [self addNavTitle:@"中国电信 / i运维"];
        [self addLSearchBar];
        ...
        [self addSearchBar];
        [self addNavigationRightButton:@"头像.png"];
        //        [self addNavLeftButton:@"mail.png"];
    ...
    dataArray1 = (NSMutableArray *)@[@"周期任务",@"故障",@"预约",@"隐患"];
...
-(void)closeBtn
{
    ...
    if (slectArray.count == 0) {
        NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
        [user setObject:@"" forKey:@"typeStr"];
    }else{
        NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
        [user setObject:inType forKey:@"typeStr"];
        
        [self nearbyResult:inType];
    }
    
    DLog(@"关闭 strMapType %@",inType);
...
- (void)didStopLocatingUser
{
    ...
    DLog(@"右下角1 == >%f",coornation1.longitude);
        ...
        NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
        NSString *str = [user objectForKey:@"typeStr"];
        if (str == nil) {
            [self nearbyResult:@"1"];
        }else{
...
- (void)didUpdateUserHeading:(BMKUserLocation *)userLocation
{
    ...
    DLog(@"%d",[search reverseGeoCode:rever]);
...
-(void)updateLocationData:(BMKUserLocation*)userLocation
{
    ...
    if (_mapView)
    {
        ...
        DLog(@"当前的坐标是: %f,%f",userLocation.location.coordinate.latitude,userLocation.location.coordinate.longitude);
    }
}
...
-(void)onGetReverseGeoCodeResult:(BMKGeoCodeSearch *)searcher result:(BMKReverseGeoCodeResult *)result errorCode:(BMKSearchErrorCode)error
{
    ...
    leftUpSmx = [NSString stringWithFormat:@"%f",coornation.longitude];
    leftUpSmy = [NSString stringWithFormat:@"%f",coornation.latitude];
    DLog(@"左上角 == >%f",coornation.latitude);
    DLog(@"左下角 == >%f",coornation.longitude);
    
    CLLocationCoordinate2D coornation1 = [_mapView convertPoint:point1 toCoordinateFromView:_mapView];//获取经纬度
    rightDownSmx = [NSString stringWithFormat:@"%f",coornation1.longitude];
    rightDownSmy = [NSString stringWithFormat:@"%f",coornation1.latitude];
    DLog(@"右下角 == >%f",coornation1.latitude);
    DLog(@"右下角 == >%f",coornation1.longitude);
    
    
    [self nearbyResult:self.strType1];
...
-(BMKAnnotationView *)mapView:(BMKMapView *)mapView viewForAnnotation:(id<BMKAnnotation>)annotation
{
    ...
    if (annotationView == nil)
    {
        ...
        if (tag == 0) {
            ...
            NSString *name = [NSString stringWithFormat:@"Lmap_icon%@.png",self.inType];
...
{
    //    DLog(@"当前位置%f,%f",userLocation.location.coordinate.latitude,userLocation.location.coordinate.longitude);
    [_mapView updateLocationData:userLocation];
}
