//
//  YZTodayWorkOrderViewController.m
//  telecom
//
...
@interface YZTodayWorkOrderViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation YZTodayWorkOrderViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"当日工单";
...
{
    NSMutableDictionary *paraDict = [NSMutableDictionary dictionary];
    paraDict[URL_TYPE] = @"commonBill/QueryCurrentBill";
    httpPOST(paraDict, ^(id result) {
        NSLog(@"%@",result);
        NSArray *listArray = [result objectForKey:@"list"];

        if ([[result objectForKey:@"result"] isEqualToString:@"0000000"] && listArray.count == 0) {
            [self showAlertWithTitle:@"提示" :@"无当日工单" :@"确定" :nil];
            return ;
        }
