...
@interface YZAcceptanceDetailViewController ()<UITableViewDataSource,UITableViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,ZYQAssetPickerControllerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UIActionSheetDelegate>
{
    //底部的scrollview
    ...
    //保存任务说明的文字高度
    CGFloat _taskDescHeight;
}
@end

@implementation YZAcceptanceDetailViewController
...
- (void)createBackgroundScrollView
{
    ...
    [self createBottomButtonWithFrame:CGRectMake(0, 810, kScreenWidth, 56)];
}
...
- (void)updateToServerIsBegin:(BOOL)isBegin checkResultId:(NSString *)checkResultId message:(NSString *)message
{
    ...
    [manager POST:requestUrl parameters:paraDict constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        if (isBegin && [result isEqualToString:@"0000000"]) {
            ...
            unqualifiedVC.dataArray = _dataArray;
            unqualifiedVC.recordArray = [responseObject objectForKey:@"list"];
            unqualifiedVC.isFromDetailVC = YES;
            unqualifiedVC.taskDescHeight = _taskDescHeight;
            unqualifiedVC.imageArray = _imageArray;
            unqualifiedVC.imageURLArray = _imageURLArray;
            unqualifiedVC.titleArray = _titleArray;
            unqualifiedVC.showMoreTitleArray = _showMoreTitleArray;
...
- (void)loadData
{
    ...
    httpPOST(para1, ^(id result) {
        ...
        for (NSString *key in array) {
            ...
            if ([key isEqualToString:@"actualStartTime"]) {
                if (![string isEqualToString:@""]) {
                    string = [string substringToIndex:13];
                }
