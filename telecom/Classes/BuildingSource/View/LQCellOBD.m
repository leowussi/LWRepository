...
@implementation LQCellOBD
+(instancetype)tableView:(UITableView *)tableView{
    static NSString *ID = @"LQCellOBD";
    LQCellOBD *cell =  [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell==nil) {
        cell = [[LQCellOBD alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self= [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.label7 = [self addLabelWithString:@"所属子类型:"];
        self.label8 = [self addLabelWithString:@"O B D 等级:"];
        self.label9 = [self addLabelWithString:@"厂           商:"];
        self.label10 = [self addLabelWithString:@"端 口  总 数:"];
        self.label11 = [self addLabelWithString:@"可用端口数:"];
        self.label12 = [self addLabelWithString:@"占用端子数:"];
        self.label13 = [self addLabelWithString:@"保留端子数:"];
        self.label14 = [self addLabelWithString:@"覆 盖  范 围:"];
        ...
        [self.rightView addSubview:self.rightLabel10];[self.rightView addSubview:self.rightLabel11];[self.rightView addSubview:self.rightLabel12];[self.rightView addSubview:self.rightLabel13];[self.rightView addSubview:self.rightLabel14];
...
-(void)setModel:(ObdModel *)model{
    _model = model;
    
    self.equmt.text  = model.obdName;
    self.rightLabel1.text  = model.obdCode;
    self.rightLabel2.text  = model.obdRegion;
    self.rightLabel3.text  = model.obdSite;
    self.rightLabel4.text  = model.obdRoom;
    self.rightLabel5.text  = model.obdRoomAddress;
    self.rightLabel6.text  = model.obdType;
    self.rightLabel7.text  = model.obdSubType;
    self.rightLabel8.text  = model.obdGrade;
    self.rightLabel9.text = model.obdManufacturer;
    self.rightLabel10.text = model.obdPortTotal;
    self.rightLabel11.text = model.obdPortAvailable;
    self.rightLabel12.text = model.obdPortTackup;
    self.rightLabel13.text = model.obdPortKeep;
    NSString *str =[model.obdRange stringByReplacingOccurrencesOfString:@"," withString:@"\n"];
    self.rightLabel14.text = str;
}
...
-(void)layoutSubviews{
    ...
    self.rightLabel4.frame=RECT(PaddingW, CGRectGetMaxY(self.rightLabel3.frame) + PaddingH, RightLabelWith, [self labelHeightWithModel:self.label4.text]+PaddingH);
    ...
    self.rightLabel14.frame=RECT(PaddingW, CGRectGetMaxY(self.rightLabel13.frame) + PaddingH, RightLabelWith, [self labelHeightWithModel:self.rightLabel14.text]+PaddingH);
    ...
    self.leftView.frame =RECT(10, temp+PaddingH, 80, CGRectGetMaxY(self.rightLabel14.frame));
    self.rightView.frame =RECT(100, temp+PaddingH, RightLabelWith+5, CGRectGetMaxY(self.rightLabel14.frame));
