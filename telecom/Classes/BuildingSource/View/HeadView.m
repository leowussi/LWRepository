...
- (id)initWithReuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithReuseIdentifier:reuseIdentifier]) {
        ...
        bgButton .titleLabel.font = [UIFont systemFontOfSize:13];
        [bgButton addTarget:self action:@selector(headBtnClick) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:bgButton];
        _bgButton = bgButton;
        
        UIImageView *im = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"p3.png"]];
        im.clipsToBounds = NO;
...
- (void)setFriendGroup:(OpenModel *)friendGroup
{
    _friendGroup = friendGroup;
    
    [_bgButton setTitle:friendGroup.gridName forState:UIControlStateNormal];
}

- (void)didMoveToSuperview
{
    _bgimageView.transform = _friendGroup.isOpened ? CGAffineTransformMakeRotation(M_PI_2) : CGAffineTransformMakeRotation(0);
}
...
- (void)layoutSubviews
{
    ...
    _bgimageView.frame = CGRectMake(self.frame.size.width - 30, 0, 20, self.frame.size.height);
}
