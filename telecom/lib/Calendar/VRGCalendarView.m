...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        [date drawInRect:CGRectMake(targetX+2, 
                                    targetY+10,
                                    kVRGCalendarViewDayWidth, 
                                    kVRGCalendarViewDayHeight) 
    ...
    //Draw markings. Edit by zhongyun:标记显示的月份不再是当月。只要有标记的月份就显示。
#if 0
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
        id markedDateObj = [self.markedDates objectAtIndex:i][@"date"];
        ...
        CGRect rectangle = CGRectMake(targetX+7, targetY-12, 32-7, 2+12);
        CGContextAddRect(context, rectangle);
        CGContextSetFillColorWithColor(context, [UIColor colorWithRed:0.0/255.0 green:155.0/255.0 blue:231.0/255.0 alpha:1.0].CGColor);
        CGContextFillEllipseInRect(context, CGRectMake(targetX+16, targetY-40, 15, 15));
        ...
        //CGRectMake(rectangle.origin.x, rectangle.origin.y+2, rectangle.size.width, 10)
        CGContextSetFillColorWithColor(context, textColor.CGColor);
        NSString* num = [self.markedDates objectAtIndex:i][@"count"];
        //超过两位数变红点
        if ([num intValue] > 99) {
        ...
        [num drawInRect:CGRectMake(targetX+16, targetY-37.5, 15, 15)
               withFont:Font(10)
          lineBreakMode:NSLineBreakByClipping 
              alignment:NSTextAlignmentCenter];
        //----------------------------------------------------------------------
