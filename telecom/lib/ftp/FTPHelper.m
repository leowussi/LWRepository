...
static Boolean MySimpleUploadByData(CFStringRef uploadDirectory, NSData *fileData, CFStringRef username, CFStringRef password, NSString *imageName)
{
    ...
    /* Copy the end of the file path and use it as the file name. */
    fileName = (CFStringRef)imageName;
    assert(fileName != NULL);
...
{	
	if (!self.uname || !self.pword) COMPLAIN_AND_BAIL(@"Please set user name and password first");
	if (!self.urlString) COMPLAIN_AND_BAIL(@"Please set URL string first");
	MySimpleUpload((CFStringRef)self.urlString, (CFURLRef)fileLocation, (CFStringRef)uname, (CFStringRef)pword);
}
...
{	
	...
	MySimpleUploadByData((CFStringRef)self.urlString, fileData, (CFStringRef)uname, (CFStringRef)pword, imageName);
}
