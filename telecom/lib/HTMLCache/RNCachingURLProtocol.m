...
- (void)startLoading
{
  if (![self useCache]) {//没使用缓存
    ...
    RNCachedData *cache = [NSKeyedUnarchiver unarchiveObjectWithFile:[self cachePathForRequest:[self request]]];//使用缓存
    if (cache) {
...
-(void)loadcache:(NSString *)request{
    RNCachedData *cache = [NSKeyedUnarchiver unarchiveObjectWithFile:request];//使用缓存
    if (cache) {
        NSData *data = [cache data];
        NSURLResponse *response = [cache response];
        NSURLRequest *redirectRequest = [cache redirectRequest];
...
- (BOOL) useCache 
{
    BOOL reachable = (BOOL) [[LQReachability reachabilityWithHostName:[[[self request] URL] host]] currentReachabilityStatus] != NotReachable;
    return !reachable;
}
